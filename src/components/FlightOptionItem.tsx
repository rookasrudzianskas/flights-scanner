//@ts-nocheck
import React from 'react';
import {Text, View, StyleSheet, TouchableOpacity} from 'react-native';
import {Ionicons} from "@expo/vector-icons";

const FlightOptionItem = ({flight}) => {
  return (
    <TouchableOpacity activeOpacity={0.7} className="flex flex-row items-center bg-white shadow-lg my-2 mx-3 rounded-[10px] p-5">
      <View className="flex-1 border border-gray-100 pr-1 gap-3">
        <View style={styles.route}>
          <Text style={styles.time}>
            {flight.from.departAt}{' '}
            <Ionicons name="airplane" size={16} color="gray" />{' '}
            {flight.from.arriveAt}
          </Text>
          <Text style={styles.airline}>{flight.from.airline}</Text>
        </View>

        <View style={styles.route}>
          <Text style={styles.time}>
            {flight.to.departAt}{' '}
            <Ionicons name="airplane" size={16} color="gray" />{' '}
            {flight.to.arriveAt}
          </Text>
          <Text style={styles.airline}>{flight.to.airline}</Text>
        </View>
      </View>

      <Text style={styles.price}>{flight.price}</Text>
    </TouchableOpacity>
  );
};

export default FlightOptionItem;

const styles = StyleSheet.create({
  route: {
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  time: {
    fontWeight: 'bold',
    fontSize: 16,
    color: 'dimgray',
    fontFamily: 'Courier New',
  },
  airline: {
    color: 'gray',
  },
  price: {
    width: 75,
    textAlign: 'center',
    fontWeight: 'bold',
    fontSize: 16,
  },
});
